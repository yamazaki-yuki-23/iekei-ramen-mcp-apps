...
読めない。順路の番号と紛れないよう、塊は面ではなく縁で色を持つ。
...
| 何               | z-index                   |
| ---------------- | ------------------------- |
| 同心円・店のピン | 400（overlayPane）        |
| 塊               | 600（markerPane）         |
| 選択中のピン     | **640**（`selectedShop`） |
| 順路の番号       | **645**（`routeOrder`）   |
| ツールチップ     | 650                       |

**順路へ寄せるのは順路が変わったときだけ。** 作り直しの 1 回目は線を引くに
留める。範囲で探した直後はユーザーが決めた画角が初期表示に入っており、そこへ
順路が割り込むと、見出しと結果は範囲のものなのに地図だけ順路へ飛ぶ
（実測: 同じ操作の 2 回目で 3 件 → 2 件になった）。
